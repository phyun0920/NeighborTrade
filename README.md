...
- **커뮤니티**: 동네 생활 게시판 및 댓글
- **반응형 디자인**: 커스텀 CSS(`common.css`) 기반 데스크톱·태블릿·모바일 최적화 UI
...
스타일은 Tailwind 없이 **`static/css/common.css` 커스텀 CSS**로 구현합니다.
...
<p>
  ...
  <img src="https://img.shields.io/badge/H2(테스트)-0066CC?style=flat-square&logoColor=white"/>
</p>

> PostGIS 경계 검증 코드는 포함되어 있으나 `app.postgis.enabled` 기본값은 `false`(미사용)입니다.
...
Repository Layer (JPA)
   ↓
Database (PostgreSQL / Supabase, H2는 테스트용)
...
| GET/POST | `/reports/{type}/{id}` | 신고 | 필요 |
...
- **Java 21**
- **Gradle** (Wrapper 포함)
- **PostgreSQL** (로컬·운영 기본) — H2는 테스트 프로필 또는 `application-secret.yml`로 대체 설정 가능
...
| 파일 | 설명 |
|------|------|
| `application.yml` | 기본 설정 (포트, JPA, UI 버전 등). DB·OAuth 등은 `application-secret.yml` 또는 환경 변수로 덮어씀 |
| `application-secret.yml` | **gitignore** — DB URL, OAuth Client ID/Secret 등 민감 정보 (로컬에서 직접 생성) |
| `application-prod.yml` | Railway·운영 프로필 (로그 레벨·SQL 출력 비활성화) |

`application-secret.yml` 예시 — **H2 인메모리로 로컬 실행할 때** (PostgreSQL 대신):
...
### 4단계 — PostgreSQL 로컬 설치 (선택)

Docker 등으로 PostgreSQL을 띄울 수 있습니다. (프로젝트 빌드·배포에는 Docker를 사용하지 않습니다.)
...
- `common.css` 커스텀 CSS 기반 반응형·관리자 UI
...
- 프로젝트 스냅샷: `doc/README-NeighborTrade_20260618목.md`
- README 검토 기록: `doc/README_검토결과_20260618.txt`
