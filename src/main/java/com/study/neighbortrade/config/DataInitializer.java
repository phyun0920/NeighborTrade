...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        Member buyer = memberRepository.findByUsername("buyer").orElseGet(() -> memberRepository.save(Member.builder().username("buyer").password(passwordEncoder.encode("buyer1234")).email("buyer@neighbortrade.local").nickname("가산구매자").role(MemberRole.ROLE_LOCAL_VERIFIED).localVerified(true).verifiedNeighborhood(gasan).mannerScore(37.0).build()));
